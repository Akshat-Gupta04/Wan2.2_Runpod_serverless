...
def get_video_data(filename, subfolder, video_type):
    ...
    Args:
        ...
        video_type (str): The type of the video (e.g., 'output').

    Returns:
        bytes: The raw video data, or None if an error occurs.
    """
    print(
        f"worker-comfyui - Fetching video data: type={video_type}, subfolder={subfolder}, filename={filename}"
    )
    data = {"filename": filename, "subfolder": subfolder, "type": video_type}
    url_values = urllib.parse.urlencode(data)
    try:
        # Use requests for consistency and timeout
        response = requests.get(f"http://{COMFY_HOST}/view?{url_values}", timeout=120)  # Longer timeout for videos
        response.raise_for_status()
...
def handler(job):
    ...
    try:
        ...
        for node_id, node_output in outputs.items():
            ...
            # Handle video outputs (from SaveVideo nodes)
            if "videos" in node_output:
                ...
                for video_info in node_output["videos"]:
                    ...
                    video_bytes = get_video_data(filename, subfolder, vid_type)
                    ...
                    if video_bytes:
                        ...
                        if os.environ.get("BUCKET_ENDPOINT_URL"):
                            try:
                                ...
                                print(f"worker-comfyui - Uploading {filename} to S3...")
                                s3_url = rp_upload.upload_image(job_id, temp_file_path)  # upload_image works for videos too
                                os.remove(temp_file_path)  # Clean up temp file
                                print(
                                    f"worker-comfyui - Uploaded {filename} to S3: {s3_url}"
                                )
    ...
    if output_data:
        # For backward compatibility, keep "images" key but it now contains both images and videos
